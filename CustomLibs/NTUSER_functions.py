...
def parse_comdlg32(reg, all=False):
    ...
    try:
        ...
        first_folder_output += display_functions.three_values("Executable", "Folder Name", "MRU Position",
                                                              first_folder_list)
        for line in first_folder_output:
            print(line)
    ...
    if not all:
        config.file_ask(outputs, "NTUSER - ComDlg32.txt")
    else:
        return outputs
...
def parse_recent_docs(reg, all=False):
    ...
    if not all:
        config.file_ask(output, "NTUSER - RecentDocs.txt")
    else:
        return output
...
def parse_user_assist(reg, all=False):
    ...
    if not all:
        config.file_ask(output, "NTUSER - UserAssist.txt")
    else:
        return output
...
def parse_typed_paths(reg, all=False):
    ...
    if not all:
        config.file_ask(output, "NTUSER - TypedPaths.txt")
    else:
        return output
...
def parse_mount_points(reg, all=False):
    ...
    if not all:
        config.file_ask(output, "NTUSER - MountPoints2.txt")
    else:
        return output
...
def parse_run(reg, all=False):
    ...
    if not all:
        config.file_ask(output, "NTUSER - Run.txt")
    else:
        return output
...
def parse_IE_urls(reg, all=False):
    ...
    if not all:
        config.file_ask(output, "NTUSER - IE Typed URLs.txt")
    else:
        return output
...
def parse_all(reg):
    output = []
    output.append(parse_recent_docs(reg, all=True))
    output.append(parse_comdlg32(reg, all=True))
    output.append(parse_user_assist(reg, all=True))
    output.append(parse_typed_paths(reg, all=True))
    output.append(parse_mount_points(reg, all=True))
    output.append(parse_run(reg, all=True))
    output.append(parse_IE_urls(reg, all=True))

    if IV.yes_or_no("Output to file? (y/n)\n"):
        with open("NTUSER - All.txt", 'w') as file:
